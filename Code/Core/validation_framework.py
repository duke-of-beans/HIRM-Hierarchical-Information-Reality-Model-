...
class HIRMValidator:
    ...
    def _validate_c_critical_threshold(self, data: Dict):
        """
        Prediction: Phase transitions occur at C â‰ˆ 8.3 Â± 0.6 bits
        """
        ...
        result1 = ValidationResult(
            ...
            prediction="â‰¥75% of with-SR runs exceed C_critical=8.3",
        ...
        if transitions_at:
            ...
            in_range = 7.7 <= mean_transition <= 8.9  # 8.3 Â± 0.6
            
            result2 = ValidationResult(
                test_name="C_critical_value",
                prediction="Transitions at 8.3 Â± 0.6 bits",
        ...
        print(f"  âœ“ Threshold exceedance: {exceed_threshold:.1%} {'PASS' if result1.passed else 'FAIL'}")
        if transitions_at:
            print(f"  âœ“ Transition value: {mean_transition:.2f} bits {'PASS' if result2.passed else 'FAIL'}")
        print(f"  âœ“ Control separation: {control_exceed:.1%} {'PASS' if result3.passed else 'FAIL'}")
    ...
    def _validate_self_reference_causality(self, data: Dict):
        ...
        print(f"  âœ“ R development: {R_increase:.1%} {'PASS' if result1.passed else 'FAIL'}")
        print(f"  âœ“ Causal effect: d={effect_size:.2f}, p={p_value:.2e} {'PASS' if result2.passed else 'FAIL'}")
        if correlations:
            print(f"  âœ“ R-C correlation: r={mean_corr:.3f} {'PASS' if result3.passed else 'FAIL'}")
    
    def _validate_criticality_emergence(self, data: Dict):
        """
        Prediction: Networks with R approach Ïƒ = 1.0
        """
        ...
        # Test 1: with-SR approaches critical (0.95 â‰¤ Ïƒ â‰¤ 1.05)
        critical_fraction = sum(0.95 <= s <= 1.05 for s in sigma_with) / len(sigma_with)
        
        result1 = ValidationResult(
            test_name="criticality_achievement",
            prediction="â‰¥60% of with-SR runs reach criticality (Ïƒ âˆˆ [0.95, 1.05])",
        ...
        # Test 2: with-SR closer to Ïƒ=1 than control
        ...
        result2 = ValidationResult(
            ...
            prediction="with-SR closer to Ïƒ=1 than control",
            observed=dist_with,
        ...
        if 'timeseries' in data:
            ...
            if correlations:
                ...
                result3 = ValidationResult(
                    ...
                    prediction="R and Ïƒ positively correlated (r > 0.5)",
                    observed=mean_corr,
                    expected=0.5,
        ...
        print(f"  âœ“ Criticality achievement: {critical_fraction:.1%} {'PASS' if result1.passed else 'FAIL'}")
        print(f"  âœ“ vs Control: {dist_with:.3f} vs {dist_control:.3f} {'PASS' if result2.passed else 'FAIL'}")
    ...
    def _validate_phase_transition_properties(self, data: Dict):
        ...
        if discontinuities:
            ...
            self.results.append(result1)
            print(f"  âœ“ Discontinuity: {mean_disc:.3f} {'PASS' if result1.passed else 'FAIL'}")
        ...
        if slowing_detected:
            ...
            result2 = ValidationResult(
                ...
                prediction="Critical slowing detected in â‰¥50% of transitions",
            ...
            self.results.append(result2)
            print(f"  âœ“ Critical slowing: {fraction_with_slowing:.1%} {'PASS' if result2.passed else 'FAIL'}")
    
    def _validate_component_multiplicativity(self, data: Dict):
        """
        Prediction: C = Î¦ Ã— R Ã— D (not additive)
        """
        print("\n[5/6] Testing multiplicative composition...")
        
        if 'timeseries' not in data:
            print("  âš  No timeseries data available")
            return
        ...
        if errors_mult and errors_add:
            ...
            print(f"  âœ“ Multiplicative error: {mean_error_mult:.4f} {'PASS' if result.passed else 'FAIL'}")
    ...
    def _validate_universality(self, data: Dict):
        ...
        if 'by_architecture' not in data:
            print("  âš  No architecture-specific data available")
            return
        ...
        if total_architectures > 0:
            ...
            result1 = ValidationResult(
                ...
                prediction="â‰¥75% of architectures show consciousness emergence",
                observed=fraction,
                expected=0.75,
            ...
            print(f"  âœ“ Universal effect: {architectures_with_effect}/{total_architectures} {'PASS' if result1.passed else 'FAIL'}")
        ...
        if len(arch_data) >= 2:
            ...
            if len(C_by_arch) >= 2:
                ...
                print(f"  âœ“ Consistency: p={p_value:.3f} {'PASS' if result2.passed else 'FAIL'}")
    ...
    def _generate_report(self) -> Dict:
        ...
        if overall_pass:
            print("âœ“ HIRM FRAMEWORK VALIDATED")
            print("  Predictions consistently confirmed across tests")
        else:
            print("âš  VALIDATION INCOMPLETE")
            print(f"  {passed}/{total} tests passed (need â‰¥{int(total*0.75)})")
