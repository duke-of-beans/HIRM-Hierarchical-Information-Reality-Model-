#!/usr/bin/env python3
"""
Comprehensive Validation Script for Ouroboros Observer Toolkit

Tests all key predictions of the theory:
1. C_critical ≈ 8.3 bits
...
def main():
    print("\n" + "=" * 80)
    print(" " * 20 + "OUROBOROS OBSERVER VALIDATION SUITE")
    print("=" * 80 + "\n")
    ...
    status = "✓ PASS" if classification['accuracy'] > 0.5 else "✗ FAIL"
    print(f"   Status: {status}\n")
    
    # C_critical prediction
    print("2. CRITICAL THRESHOLD PREDICTION (C_critical ≈ 8.3 bits)")
    print("-" * 80)
    c_crit = results['C_critical']
    print(f"   Theoretical C_critical: {c_crit['theoretical_C_critical']:.1f} bits")
    print(f"   Measured C_critical: {c_crit['mean_C_critical']:.2f} ± {c_crit['std_C_critical']:.2f} bits")
    ...
    status = "✓ PASS" if deviation_ok else "⚠ MARGINAL"
    print(f"   Status: {status}\n")
    ...
    print(f"   Mean branches: {branches['mean_branches']:.1f} ± {branches['std_branches']:.1f}")
    ...
    status = "✓ PASS" if branches['in_range'] else "⚠ MARGINAL"
    print(f"   Status: {status}\n")
