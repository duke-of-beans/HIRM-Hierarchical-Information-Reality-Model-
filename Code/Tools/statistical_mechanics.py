...
class IsingBrain:
    ...
    - Consciousness calculation (C = Î¦ Ã— R Ã— D)
    ...
    def compute_phi(self) -> float:
        ...
        Uses bipartition mutual information as proxy for Î¦.
        """
        mid = self.N // 2
        ...
        # Mutual information (upper bound on Î¦)
        phi = H1 + H2 - H_total
        return max(0, phi)
    ...
    def compute_dimensionality(self) -> float:
        ...
        Uses participation ratio: D = (Î£a_i)Â² / Î£a_iÂ²
    ...
    def consciousness(self) -> float:
        """
        Consciousness measure C(t) = Î¦(t) Ã— R(t) Ã— D(t)
        """
        ...
        while frontier:
            ...
            for j in neighbors:
                if j not in cluster and self.spins[i] == self.spins[j]:
                    # Add with probability 1 - exp(-2Î²|J_ij|)
...
class Visualizer:
    ...
    def plot_phase_transition(
        ...
        axes[0, 1].set_ylabel('Susceptibility Ï‡', fontsize=12)
        ...
        axes[1, 0].set_ylabel('Energy âŸ¨EâŸ©', fontsize=12)
        ...
        if T_c:
            ...
            axes[1, 2].text(0.5, 0.52, f'C_critical â‰ˆ {T_c:.2f} bits', 
    ...
    def plot_critical_scaling(
        ...
        if np.sum(mask) > 0 and 'beta' in exponents:
            ...
            axes[0].loglog(t_fit, m_fit * m[mask][0] / m_fit[0], '--', 
                          label=f'Î² = {beta_exp:.3f} Â± {beta_err:.3f}')
        ...
        if np.sum(mask) > 0 and 'gamma' in exponents:
            ...
            axes[1].loglog(t_fit, chi_fit * chi[mask][0] / chi_fit[0], '--',
                          label=f'Î³ = {gamma_exp:.3f} Â± {gamma_err:.3f}')
            
            axes[1].set_xlabel('|T - T_c|', fontsize=12)
            axes[1].set_ylabel('Susceptibility Ï‡', fontsize=12)
        ...
        axes[2].bar(x - width/2, beta_theory, width, label='Theoretical Î²', alpha=0.7)
        axes[2].bar(x + width/2, gamma_theory, width, label='Theoretical Î³', alpha=0.7)
        ...
        if 'beta' in exponents and 'gamma' in exponents:
            ...
            axes[2].axhline(beta_measured, color='C0', linestyle='--', 
                           label=f'Measured Î² = {beta_measured:.3f}')
            axes[2].axhline(gamma_measured, color='C1', linestyle='--',
                           label=f'Measured Î³ = {gamma_measured:.3f}')
...
def run_complete_analysis(
    ...
    print("\nUniversality Class Comparison:")
    print("  Mean-field:  Î²=0.500, Î³=1.000, Î½=0.500")
    print("  2D Ising:    Î²=0.125, Î³=1.750, Î½=1.000")
    print("  3D Ising:    Î²=0.326, Î³=1.237, Î½=0.630")
    print("  Percolation: Î²=0.410, Î³=1.800, Î½=0.875")
