...
def beta_functions(x: np.ndarray, ell: float, params: Dict) -> np.ndarray:
    """
    Complete Î²-function system for HIRM consciousness parameters.
    ...
    --------
    dxdell : array
        Derivatives [dC/dâ„“, dÎ¦/dâ„“, dR/dâ„“, dD/dâ„“]
    """
    C, Phi, R, D = x
...
def find_fixed_point(params: Dict, 
    ...
    """
    Find RG fixed point by solving Î²(x*) = 0.
...
def compute_jacobian(x_star: np.ndarray, params: Dict, eps: float = 1e-6) -> np.ndarray:
    """
    Compute Jacobian matrix at fixed point: J_ij = âˆ‚Î²_i/âˆ‚x_j
...
def extract_critical_exponents(C_data: np.ndarray,
    ...
    epsilon_data : array  
        Reduced distance from critical point: Îµ = (C - C*)/C*
    xi_data : array, optional
        Correlation lengths
    M_data : array, optional
        Order parameter (e.g., Î¦ - Î¦*)
    ...
    if 'nu' in exponents and 'gamma' in exponents:
        ...
        print(f"  Î³ = Î½(2 - Î·)")
        print(f"  LHS: Î³ = {exponents['gamma']:.3f}")
        print(f"  RHS: Î½(2 - Î·_est) = {exponents['nu'] * (2 - eta_estimated):.3f}")
...
def plot_rg_flow_phase_diagram(initial_conditions: List[np.ndarray],
    ...
    for i, x0 in enumerate(initial_conditions):
        ...
        ax_combined.plot(ell_range, traj_norm[:, 1], color=colors[i],
                        ...
                        label='Î¦/Î¦*' if i==0 else '')
    
    # Fixed point markers
    for ax, val, label in [(ax_C, C_star, 'C*'), (ax_Phi, Phi_star, 'Î¦*'),
    ...
    ax_Phi.set_ylabel('Î¦ (bits)', fontsize=12, fontweight='bold')
    ...
    for ax in [ax_C, ax_Phi, ax_R, ax_D]:
        ax.set_xlabel('Scale â„“ = log(Î›/Î›â‚€)', fontsize=11)
        ax.grid(alpha=0.3)
        ax.legend(fontsize=9)
    
    ax_CPhi.set_xlabel('C (bits)', fontsize=12)
    ax_CPhi.set_ylabel('Î¦ (bits)', fontsize=12)
    ax_CPhi.set_title('C-Î¦ Phase Space', fontsize=13, fontweight='bold')
    ...
    ax_combined.set_xlabel('Scale â„“ = log(Î›/Î›â‚€)', fontsize=12)
