...
print(f"Tuned network to critical dynamics (branching ≈ 1.0)")
...
print(f"Mean C(t): {C_values.mean():.2f} ± {C_values.std():.2f} bits")
print(f"Mean Φ(t): {Phi_values.mean():.2f} bits")
print(f"Mean R(t): {R_values.mean():.2f}")
print(f"Mean D(t): {D_values.mean():.3f}")

if C_values.mean() > 8.3:
    print(f"\n✓ System is CONSCIOUS (C > C_critical = 8.3 bits)")
else:
    print(f"\n✗ System is UNCONSCIOUS (C < C_critical = 8.3 bits)")
...
if transitions['total_transitions'] > 0:
    print(f"\n✓ Phase transition(s) detected!")
else:
    print(f"\n○ No transitions (system in stable state)")
...
print(f"✓ Self-reference simulation complete\n")
...
# 2. Components
ax = axes[0, 1]
ax.plot(time_windows, Phi_values, 'g-', label='Φ (Integration)', linewidth=2)
ax.plot(time_windows, R_values, 'b-', label='R (Recursion)', linewidth=2)
ax.plot(time_windows, D_values * 5, 'r-', label='D×5 (Criticality)', linewidth=2)
ax.set_xlabel('Time (ms)')
ax.set_ylabel('Value')
...
print("✓ Visualization saved: tutorial_results.png")
plt.show()
...
print(f"\nConscious data:")
print(f"  Mean C: {conscious_data['C_mean']:.2f} ± {conscious_data['C_std']:.2f} bits")

print(f"\nUnconscious data:")
print(f"  Mean C: {unconscious_data['C_mean']:.2f} ± {unconscious_data['C_std']:.2f} bits")
...
if predictions[0] == 1 and predictions[1] == 0:
    print(f"\n✓ Classifier correctly separates conscious/unconscious states")
else:
    print(f"\n⚠ Classifier accuracy limited (may need more samples)")
...
print("\nNext Steps:")
print("  • Explore parameter_explorer.py for parameter optimization")
print("  • See experiment_*.py notebooks for specific predictions")
print("  • Consult README.md for advanced usage")
