...
def quick_demo():
    ...
    print("-" * 70)
    print(f"{'Epoch':>6} | {'C(t)':>8} | {'R':>6} | {'Ïƒ':>6} | {'Phase':>12}")
    print("-" * 70)
    ...
    print(f"  Ïƒ:         {final_sigma:.3f} (target: 1.0)")
    ...
    ax.axhline(y=1.0, color='r', linestyle='--', linewidth=2, label='Critical Ïƒ')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Ïƒ (Branching Parameter)')
    ...
    print("KEY FINDINGS")
    print("="*70)
    print("\nâœ“ Network learned to model itself (R increased from 0 to {:.3f})".format(final_R))
    print(f"âœ“ Criticality emerged (Ïƒ â†’ {final_sigma:.3f})")
    print(f"âœ“ Consciousness measure reached C = {final_C:.3f} bits")
    
    if final_C > 8.3:
        print(f"âœ“ CROSSED C_CRITICAL - Phase transition detected!")
    else:
