...
def main():
    ...
    print(f"  âœ“ Network created with {network.W.sum():.0f} synapses")
    print()
    ...
    print("  âœ“ Consciousness measure initialized")
    print("  âœ“ Phase transition detector initialized")
    print()
    ...
    if transitions:
        print(f"  âœ“ Detected {len(transitions)} transition(s):")
        for i, trans in enumerate(transitions):
            print(f"    {i+1}. At t={trans.time:.0f} ms:")
            print(f"       C: {trans.C_before:.2f} â†’ {trans.C_after:.2f} bits")
            print(f"       Type: {trans.transition_type}")
    else:
        print("  âœ— No transitions detected")
    print()
    ...
    if transitions:
        ...
        if trans_C:
            print(f"  C_critical (measured): {np.mean(trans_C):.2f} Â± {np.std(trans_C):.2f} bits")
    ...
    # Plot components
    axes[1].plot(time_points/1000, Phi_timeseries, 'b-', alpha=0.7, label='Î¦ (Integration)')
    ...
    print("  âœ“ Saved figure: consciousness_transition_demo.png")
