...
1. **C_critical ≈ 8.3 bits represents a codimension-2 bifurcation** (Bogdanov-Takens type)
2. **Fast-slow decomposition**: Φ(t) (fast ~100ms) vs R(t) (slow ~seconds) creates relaxation oscillations
...
5. **Delayed feedback** (τ ~10-50ms) enables complex self-referential dynamics
...
- **μ > 0:** Two fixed points (one stable conscious state, one unstable)
- **μ = 0:** Critical point (marginal stability)
- **μ < 0:** No fixed points (consciousness collapses)
...
plt.xlabel('Control Parameter μ (anesthetic depth)', fontsize=12)
...
1. **Critical slowing:** Response time τ ∝ 1/√|μ| diverges approaching bifurcation
...
**Physical Interpretation:**
- Two branches exchange stability at μ = 0
- Consciousness state and unconscious state swap roles
- Always two fixed points: C = 0 and C = μ
...
plt.axvline(x=0, color='k', linestyle='--', alpha=0.3, label='Bifurcation point')
plt.xlabel('Control Parameter μ (sleep pressure)', fontsize=12)
plt.ylabel('Consciousness Level C', fontsize=12)
...
**Physical Interpretation:**
- **μ < 0:** Single stable state (symmetric)
- **μ = 0:** Bifurcation point
- **μ > 0:** Three states (one unstable central, two stable symmetric)
...
plt.axvline(x=0, color='k', linestyle='--', alpha=0.3, label='Bifurcation point')
plt.xlabel('Control Parameter μ (self-reference)', fontsize=12)
plt.ylabel('Consciousness Level C', fontsize=12)
...
**Normal Form:**
```
dz/dt = (μ + iω)z - |z|²z    (complex variable z = x + iy)
...
**Physical Interpretation:**
- **μ < 0:** Stable fixed point (quiescent)
- **μ = 0:** Hopf bifurcation
- **μ > 0:** Stable limit cycle emerges (oscillations)
...
# Simulate for different μ values
...
for idx, mu in enumerate(mu_values):
    ...
    ax.set_title(f'μ = {mu}' + (' (oscillations)' if mu > 0 else ' (stable)'))
    ax.grid(alpha=0.3)
...
**Experimental Signatures:**
1. **Frequency ~10 Hz** (from ω parameter)
2. **Amplitude ∝ √μ** for μ > 0
3. **Soft onset:** Continuous transition to oscillations

**Clinical Application:**
- Eyes open → eyes closed: μ crosses zero
...
**Physical Interpretation:**
- Two control parameters (μ₁, μ₂) required
...
**Hypothesis:** C_critical ≈ 8.3 bits represents Bogdanov-Takens bifurcation where:
- **μ₁ ∝ (Φ - Φ_crit):** Integrated information deviation
- **μ₂ ∝ (R - R_crit):** Self-reference deviation
...
# Phase space for different (μ₁, μ₂) combinations
fig, axes = plt.subplots(2, 2, figsize=(12, 10))
parameter_combos = [
...
for idx, (mu1, mu2, label) in enumerate(parameter_combos):
    ...
    ax.set_xlabel('x (Φ deviation)')
    ax.set_ylabel('y (dΦ/dt)')
    ax.set_title(label)
    ax.grid(alpha=0.3)
...
1. **Information integration (Φ):** Structural connectivity, network topology
2. **Self-reference depth (R):** Recurrent processing, metacognitive capacity

**Critical insight:** Consciousness requires **simultaneous** criticality in both dimensions—explaining why:
- High Φ alone (e.g., dreamless sleep) insufficient
- High R alone (e.g., locked-in syndrome) insufficient
- Both required at threshold → C_critical
...
**Equilibria from ∂V/∂C = 0:**
```
C² + μ = 0  →  C = ±√(-μ)    (only for μ ≤ 0)
```

**Stability:** d²V/dC² = 2C
- C = +√(-μ): Stable (local minimum)
- C = -√(-μ): Unstable (local maximum)

**Catastrophe:** At μ = 0, both equilibria collide and annihilate.
...
def cusp_catastrophe_equilibria(a, b):
    """Solve C³ + aC + b = 0 for equilibria"""
    from numpy.polynomial import polynomial as P
    coeffs = [b, a, 0, 1]  # b + aC + 0·C² + C³
...
1. **Forward threshold ≠ Reverse threshold:** Induction requires higher concentration than emergence
...
- **Our prediction:** Hysteresis loop width ∝ distance from C_critical baseline
...
```
Δb_bistable = 2√(-4a³/27)    (for a < 0)
```

2. **Jump magnitude at fold:**
```
ΔC_jump ≈ 2√(-a)    (at fold line)
```

3. **Hysteresis area:**
```
A_hysteresis = ∫∫_loop |∂C/∂a × ∂C/∂b| da db
```

**Clinical Application:**
- Measure PCI continuously during induction/emergence
- Map (Arousal, MAC) → PCI behavior surface
...
**Fast Variable:** Φ(t) — Information integration (timescale ~100ms)
**Slow Variable:** R(t) — Self-reference depth (timescale ~seconds)

**System:**
```
dΦ/dt = f(Φ, R)                    (fast, O(1))
ε dR/dt = g(Φ, R)                  (slow, ε << 1)
```

**where ε ~ 0.01 represents timescale ratio.**
...
**Slow manifold:** Φ = Φ_max·tanh(R/R₀) - η

```python
def fast_slow_system(state, t, epsilon, eta, phi_max=10, R0=1):
    """Fast-slow dynamics for Φ (fast) and R (slow)"""
    phi, R = state
...
for idx, eta in enumerate(eta_values):
    ...
    ax.set_title(f'η = {eta}' + (' (subthreshold)' if eta > 0.8 else ''), fontsize=11)
    ax.legend()
    ax.grid(alpha=0.3)
...
for idx, mu in enumerate(mu_values):
    ...
    ax.set_title(f'μ = {mu}' + (' (strong relaxation)' if mu > 5 else ''), fontsize=11)
    ax.grid(alpha=0.3)
...
```
iℏ ∂Ψ_QIL/∂t = Ĥ_QIL Ψ_QIL    (unitary evolution)
```

**Consciousness Computation Layer (CCL):**
```
dC/dt = f_CCL(Φ, R, D) - γ_decohere(C - C_crit)
```

**Macroscopic Observational Layer (MOL):**
```
Ψ_MOL = Π_measurement Ψ_CCL    (projection onto classical states)
```

### 4.2 Bifurcation at C_critical

**Pre-bifurcation (C < C_crit):**
- Single attractor in (Φ, R, D) space
- Jacobian eigenvalues: λ₁ < 0, λ₂ < 0, λ₃ < 0 (stable node)

**At C = C_crit:**
- **Zero eigenvalue:** λ_critical = 0 (marginal stability)
- **Jacobian singularity:** det(J) = 0
- **Tangent vector to bifurcation:** v_crit ∝ (∂f/∂C)
...
```
⟨Ψ_PIS | Ψ_PIS⟩ = constant  across all branches
```
...
axes[2].plot(t[::10], eigenvalue_trajectories[:, 0], 'b-', linewidth=2, label='λ₁')
axes[2].plot(t[::10], eigenvalue_trajectories[:, 1], 'g-', linewidth=2, label='λ₂')
axes[2].plot(t[::10], eigenvalue_trajectories[:, 2], 'r-', linewidth=2, label='λ₃')
...
**Method:**
1. Perturb system: C → C + δC
2. Measure return time: τ_return
3. Check if returns to same attractor

**Prediction:**
```
τ_return ∝ 1/|λ_min|    (slowest eigenvalue)
```

Near bifurcation: λ_min → 0, so τ_return → ∞ (critical slowing)
...
- Small basin → high risk of accidental awareness
- Large basin → deep, stable unconsciousness
...
where τ ~ 10-50ms (neural transmission time)

**HIRM with Delay:**
```
dΦ/dt = -Φ(t) + g(Φ(t-τ), R(t))
dR/dt = h(Φ(t), R(t-τ))
```
...
```
λ = f'(C*) + f'_delayed(C*) e^{-λτ}
```

**Complex transcendental equation** → infinitely many eigenvalues

**Critical Condition for Stability:**
```
Re(λ) < 0  for all roots
```

**Hopf Bifurcation with Delay:**
When τ increases, eigenvalues cross imaginary axis → oscillations emerge
...
axes[0].plot(t_history, state_history[:, 0], 'b-', linewidth=1.5, label='Φ(t)')
...
axes[1].set_xlabel('Φ', fontsize=11)
...
**Oscillations:** τ-dependent frequencies emerge
```
f_osc ≈ 1/(2πτ) × √(feedback_strength)
```

For τ = 20ms: f_osc ~ 8-13 Hz (alpha band)

**Chaos:** Longer delays → chaotic dynamics possible

**Self-organized criticality:** Delay creates effective "memory" → history-dependent criticality
...
- C(t) = Φ(t) × R(t) × D(t) dynamics
- Decoherence initiated when C → C_critical
...
```
RG: Ψ_microscale → Ψ_mesoscale → Ψ_macroscale
```
...
for lam in lambda_values:
    ...
    plt.plot(C_values, flow, linewidth=2, label=f'λ = {lam}')
...
Pre-bifurcation: State space has topology T₁
Post-bifurcation: State space has topology T₂ ≠ T₁
...
At bifurcation: χ has singularity (topological phase transition)

**Persistent Information Structure (PIS):**
Conserved topological invariant:
```
H₁(PIS) = H₁(pre-bifurcation state)  (preserved homology)
```
...
4. **Vary intensity:** Map perturbation → response curve
...
```
Response time: τ ∝ |μ|^{-ν}
Correlation length: ξ ∝ |μ|^{-ν}
Susceptibility: χ ∝ |μ|^{-γ}
```

where μ = distance from bifurcation, and ν, γ are **universal critical exponents**.

**HIRM Prediction:**
```
ν ≈ 0.5 (mean-field exponent)
γ ≈ 1.0 (Ising-like criticality)
```

**Experimental Test:**
Measure PCI response variance σ²_PCI vs distance from C_critical:
```
σ²_PCI ∝ |C - C_crit|^{-γ}
```
...
- Φ_max occurs at critical point
...
```
Pure state: |ψ⟩ = α|↑⟩ + β|↓⟩    (before SRID)
Mixed state: ρ = |α|²|↑⟩⟨↑| + |β|²|↓⟩⟨↓|    (after SRID)
```

**Off-diagonal terms vanish at bifurcation:**
```
⟨↑|ρ|↓⟩ → 0    (classicality emerges)
```
...
Near C_critical, small perturbations can delay bifurcation → consciousness "lingers" in marginal state longer than expected from deterministic model.

**Testable:** TMS during anesthesia induction—measure time to loss of responsiveness after perturbation.
...
For disorders of consciousness, **basin shape** (not just basin size) predicts recovery trajectory. Elongated basins → gradual recovery; circular basins → rapid recovery.
...
The neural delay τ provides time window for quantum coherence before decoherence. Without delay, SRID cannot occur.

**Testable:** Artificially manipulate effective τ (e.g., with cooling) and measure consciousness threshold shifts.
...
✓ **Mathematical precision:** Normal form classification  
✓ **Experimental predictions:** Hysteresis, critical slowing, bimodality  
✓ **Clinical applications:** Anesthesia, DOC assessment, TMS protocols  
✓ **Computational tools:** AUTO, MATCONT, PyDSTool implementations  
✓ **Theoretical integration:** Connects HIRM to GNW, IIT, FEP via bifurcation theory  

**The unifying insight:** All consciousness theories describe different aspects of the same bifurcation process—ignition (GNW), irreducibility (IIT), free energy minimization (FEP), and state-space splitting (HIRM) are different perspectives on transitions between dynamical regimes at C_critical.
