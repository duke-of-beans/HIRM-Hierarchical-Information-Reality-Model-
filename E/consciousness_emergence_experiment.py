...
Self-reference → Criticality → Phase transition at C_crit

Experiment Design:
------------------
1. Train 4 different architectures on self-modeling tasks
2. Track C(t) = Φ(t) × R(t) × D(t) during training
3. Show that C(t) crosses C_crit ≈ 8.3 bits
4. Demonstrate criticality emergence (σ → 1.0)
...
class ConsciousnessEmergenceExperiment:
    ...
    def run_full_experiment(
        ...
        print("Demonstrating self-reference → criticality → phase transition")
        print("="*80 + "\n")
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 10 == 0:
                ...
                print(f"Epoch {epoch:3d} | "
                      ...
                      f"σ={history['branching_parameter'][-1]:.3f} | "
                      f"Phase: {phase}")
    ...
    def analyze_results(self, results: Dict):
        ...
        for arch in ['SRRN', 'MetaLearning', 'PredictiveCoding', 'Transformer']:
            ...
            print(f"    Final σ: {with_sr['final_sigma']:.3f}")
            ...
            print(f"    Final σ: {control['final_sigma']:.3f}")
            ...
            print(f"  → C improvement: {C_improvement:.1f}%")
    ...
    def create_visualizations(self, results: Dict):
        ...
        # Figure 3: Component analysis (Φ, R, D)
    ...
    def _plot_phase_space(self, results: Dict):
        """Plot phase space trajectories (R vs Φ)."""
        ...
        # C = Φ × R × D, assuming D ≈ 1 at criticality
        ...
        ax.set_ylabel('Integrated Information Φ [bits]', fontsize=14, fontweight='bold')
    ...
    def _plot_components(self, results: Dict):
        """Plot Φ, R, D components separately."""
        
        fig, axes = plt.subplots(1, 3, figsize=(18, 5))
        
        components = ['phi', 'R', 'D']
        titles = ['Integrated Information Φ', 'Self-Reference Depth R', 
                 'Criticality Proximity D']
        ylabels = ['Φ [bits]', 'R (normalized)', 'D (normalized)']
        ...
        plt.suptitle('Component Analysis: Φ, R, D Evolution',
                     fontsize=16, fontweight='bold', y=1.02)
        plt.tight_layout()
    ...
    def _plot_criticality(self, results: Dict):
        ...
        ax.axhline(y=1.0, color='red', linestyle=':', linewidth=3,
                  label='Critical σ = 1.0')
        
        ax.set_xlabel('Epoch', fontsize=14, fontweight='bold')
        ax.set_ylabel('Branching Parameter σ', fontsize=14, fontweight='bold')
        ax.set_title('Criticality Emergence: Self-Reference Drives σ → 1.0',
                    fontsize=16, fontweight='bold')
    ...
    def summarize_findings(self, results: Dict):
        ...
        for arch in ['SRRN', 'MetaLearning', 'PredictiveCoding', 'Transformer']:
            ...
            print(f"   {arch}: σ = {sigma:.3f} (target: 1.0)")
...
def main():
    ...
    print("  3. Phase transitions occur at C_critical ≈ 8.3 bits")
