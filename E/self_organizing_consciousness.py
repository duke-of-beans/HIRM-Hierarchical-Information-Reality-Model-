...
4. Show phase transitions at C_crit ≈ 8.3 bits
...
class SelfReferentialRecurrentNetwork(nn.Module):
    ...
    def compute_criticality_metrics(
        ...
        # σ = <s_t+1> / <s_t> where s is number of active units
...
class ConsciousnessTracker:
    ...
    C(t) = Φ(t) × R(t) × D(t) and detect phase transitions.
    """
    ...
    def update(
        ...
        # Using approximation: Φ ≈ mutual information between parts
    ...
    def _estimate_phi(self, hidden_states: torch.Tensor) -> float:
        ...
        Φ ≈ MI(X; Y) where X, Y are partitions
        ...
        # Simple approximation: correlation-based Φ
        # Φ measures how much parts constrain each other
        ...
        try:
            ...
            # MI ≈ -0.5 * log(det(Σ)) + 0.5 * log(det(Σ1)) + 0.5 * log(Σ2))
